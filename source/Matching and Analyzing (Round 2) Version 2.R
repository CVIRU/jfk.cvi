...
length(unique(matchrows[matchrows[,3]==0,2]))
# 
# View(matchrows)
...
###use this for loop to find the seed that produces the most unique matches, 998
#for (k in 1:1000){
...
# if (length(unique(matchrow.final[matchrow.final[,3]==0,2]))>=26){
# 
#   print(k)
#   print(length(unique(matchrow.final[matchrow.final[,3]==0,2])))
# 
# }
# 
# }
...
for (i in unique(matchrow.final[matchrow.final[,3]==0,2])[3:length(unique(matchrow.final[matchrow.final[,3]==0,2]))]){
...
#(10, 20, 20, 30, 1, 2) yields 43 matches

#(5, 20, 20, 30, 1, 2) yields 38 matches
...
follow.up.analysis(ScoreNum=3, FollowUpNum=7)$p.value
...
lmer.analysis=function(AgeNum = 5, BaselineMobNum=20, BaselineActNum=20, 
                       BaselineCogNum=30, PScoreNum = 1, FacAdjNum = 2,
                      ScoreNum=1, AverageRange=c(0,100,200)){
...
word=lmer.analysis(ScoreNum=1, AverageRange=c(0,130,200))

(summary(word))$coefficients
 
word=lmer.analysis(ScoreNum=2, AverageRange=c(0,130,200))

(summary(word))$coefficients

word=lmer.analysis(ScoreNum=3, AverageRange=c(0,130,200))
...
for (j in 1:3){
  ...
  plot(-100,-100, xlim=c(-50,365), ylim=c(-30,130), ylab=paste(ScoreName[j], "Score"), 
       xlab="Days Since Group Assignment",
       main=paste(ScoreName[j], "Score by Days Since Group Assignment for Study Group"))
  
  for (i in StudyGroupIDs){
    
    if (i!=103 | i!=212){
      
      points(Interpolate.Master[((Interpolate.Master[,"ID"]==i & (Interpolate.Master[,"DaysId"]==1 | Interpolate.Master[,"DaysId"]==2))), "Days.After.Assignment"],
      ...
      points(Interpolate.Master[(Interpolate.Master[,"ID"]==i & Interpolate.Master[,"DaysId"]<=8 & Interpolate.Master[,"DaysId"]>=4 ),"Days.After.Assignment"],
             Interpolate.Master[(Interpolate.Master[,"ID"]==i & Interpolate.Master[,"DaysId"]<=8 & Interpolate.Master[,"DaysId"]>=4 ),ScoreVarName[j]], 
             pch=19, col="green")
      
      lines(c(Interpolate.Master[((Interpolate.Master[,"ID"]==i & (Interpolate.Master[,"DaysId"]==1 | Interpolate.Master[,"DaysId"]==2))), "Days.After.Assignment"],
              Interpolate.Master[((Interpolate.Master[,"ID"]==i & Interpolate.Master[,"DaysId"]==1)), "Zero"],
              Interpolate.Master[(Interpolate.Master[,"ID"]==i & Interpolate.Master[,"DaysId"]<=8 & Interpolate.Master[,"DaysId"]>=4 ),"Days.After.Assignment"]),
  ...
  legend("topright", legend=c("Scores before Assignment", "Score at Assignment", "Study Group"),
         col=c("blue", "purple", "green"), pch=c(19,19,19), cex=0.65)
   
  abline(v=1, col="black")
  abline(v=130, col="black")
  abline(v=200, col="black")

  #plot for Control Group
  plot(-100,-100, xlim=c(-50,365), ylim=c(-30,130), ylab=paste(ScoreName[j], "Score"), 
       xlab="Days Since Group Assignment",
       main=paste(ScoreName[j], "Score by Days Since Group Assignment for Control Group"))
...
points(Interpolate.Master[(Interpolate.Master[,"ID"]==269 & Interpolate.Master[,"DaysId"]<=8 & Interpolate.Master[,"DaysId"]>=4 ),"Days.After.Assignment"],
       Interpolate.Master[(Interpolate.Master[,"ID"]==269 & Interpolate.Master[,"DaysId"]<=8 & Interpolate.Master[,"DaysId"]>=4 ),ScoreVarName[j]], 
       pch=19, col="green")

points(Interpolate.Master[(Interpolate.Master[,"ID"]==111 & Interpolate.Master[,"DaysId"]<=8 & Interpolate.Master[,"DaysId"]>=4 ),"Days.After.Assignment"],
       Interpolate.Master[(Interpolate.Master[,"ID"]==111 & Interpolate.Master[,"DaysId"]<=8 & Interpolate.Master[,"DaysId"]>=4 ),ScoreVarName[j]], 
       pch=19, col="red")

legend("topright", legend=c("Scores before Assignment", "Score at Assignment", "Control Group", "Study Group"),
...
##############################ANALYSIS OF BINARY PAYIENT OUTCOMES##################################
...
demo.chart=as.data.frame(matrix(NA,60,2))
...
colnames(demo.chart)=c("Study Group", "Control Group")
...
t.test(NewMaster.One[NewMaster.One[,"Group"]=="Study Group", "Age"],
        NewMaster.One[NewMaster.One[,"Group"]=="Control Group", "Age"])
...
prop.test(table(NewMaster.One[NewMaster.One[,"Group"]=="Study Group" | 
                                NewMaster.One[,"Group"]=="Control Group", "Group"], 
                NewMaster.One[, "Gender"])[-2:-3,], correct=FALSE)
...
prop.test(table(NewMaster.One[NewMaster.One[,"Group"]=="Study Group" | 
                                NewMaster.One[,"Group"]=="Control Group", "Group"], 
                NewMaster.One[, "Deceased_Y.N"])[-2:-3,], correct=FALSE)
...
chisq.test(table(NewMaster.One[,c("New.Race", "Group")])[,c(1,4)])
...
prop.test(table(NewMaster.One[NewMaster.One[,"Group"]=="Study Group" | 
                                NewMaster.One[,"Group"]=="Control Group", "Group"], 
                NewMaster.One[, "Hispanic.Ethnicity"])[-2:-3,-2], correct=FALSE)
...
chisq.test(table(NewMaster.One[,c("Health.Insurance.Name", "Group")])[,c(1,4)])
...
t.test(NewMaster.One[NewMaster.One[,"Group"]=="Study Group" & NewMaster.One[,"Education.Level"]<100, "Education.Level"],
       NewMaster.One[NewMaster.One[,"Group"]=="Control Group" & NewMaster.One[,"Education.Level"]<100, "Education.Level"])
...
prop.test(table(NewMaster.One[NewMaster.One[,"Group"]=="Study Group" | 
                                NewMaster.One[,"Group"]=="Control Group", "Group"], 
                NewMaster.One[, "Hemiparesis_Left"])[-2:-3,], correct=FALSE)
...
prop.test(table(NewMaster.One[NewMaster.One[,"Group"]=="Study Group" | 
                                NewMaster.One[,"Group"]=="Control Group", "Group"], 
                NewMaster.One[, "Hemiparesis_Right"])[-2:-3,], correct=FALSE)
...
prop.test(table(NewMaster.One[NewMaster.One[,"Group"]=="Study Group" | 
                                NewMaster.One[,"Group"]=="Control Group", "Group"], 
                NewMaster.One[, "Hemiparesis_Bilateral"])[-2:-3,], correct=FALSE)
...
for (i in 13:16){
  ...
  print(prop.test(table(NewMaster.One[NewMaster.One[,"Group"]=="Study Group" | 
                                  NewMaster.One[,"Group"]=="Control Group", "Group"], 
                  NewMaster.One[, rownames(demo.chart)[i]])[-2:-3,], correct=FALSE))
...
for (i in c(17:19,21:28)){
  ...
  print(t.test(NewMaster.One[NewMaster.One[,"Group"]=="Study Group" & NewMaster.One[,rownames(demo.chart)[i]]<200, rownames(demo.chart)[i]],
         NewMaster.One[NewMaster.One[,"Group"]=="Control Group" & NewMaster.One[,rownames(demo.chart)[i]]<200, rownames(demo.chart)[i]]))
...
for (i in 52:57){
  ...
  print(t.test(StudySet[StudySet[,i-51]<1000,i-51], 
               ControlSet[ControlSet[,i-51]<1000,i-51]))
...
for (i in 58:60){
  ...
  demo.chart[i,"Study Group"]=mean(NewMaster.One[NewMaster.One[,"Group"]=="Study Group",i], na.rm=TRUE)
  
  demo.chart[i,"Control Group"]=mean(NewMaster.One[NewMaster.One[,"Group"]=="Control Group",i], na.rm=TRUE)
