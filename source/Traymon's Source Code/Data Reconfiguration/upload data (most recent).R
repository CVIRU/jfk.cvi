...
length(unique(OldMaster[, "ID"]))
# As of 1/10/2019 945 patients remain after removing those that were pregnant
...
for (i in 1:dim(Master)[1]){
  ...
  # change missing Education.Level to NA
  if(Master[i, "Education.Level"] == 9999
     | Master[i, "Education.Level"] == 8888
     | is.na(Master[i, "Education.Level"]) == 1){
    
    Master[i, "Education.Level"] = NA
  ...
  for (j in c("Height",
              ...
              "ACHosp.Stroke.Severity.Number_NIHSS",
              "ACHosp.Stroke.Severity.Number_ICH",
              "ACHosp.Stroke.Severity.Number_HH",
              "BMI",
              "SBP.HTN",
              "DBP.HTN",
              "Fast.Nonfasting.LDL",
...
group.check.table = data.frame(Group = c("Pregnant",
                               ...
                               Number = c(length(preg.IDs),
                                          length(StudyGroupIDs)-length(noCVG.IDs),
                                          length(ControlGroupIDs),
                                          length(other.IDs),
                                          length(noCVG.IDs)))
...
both.IDs = unique(NewMaster[which(NewMaster[, "ID"] %in% intersect(StudyGroupIDs, ControlGroupIDs)), "ID"])
NewMaster = NewMaster[-which(NewMaster[, "ID"] %in% intersect(StudyGroupIDs, ControlGroupIDs)), ]
# As of 1/10/2019 there is 1 patient in both groups
...
# As of 1/10/2019 one patient is missing DaysId 3
