#################################Program Description################################
#Name: Descriptive Chart (All Patients) (end of 2017 data)                         #
...
for.names = colnames(Interpolate.Master.One)[c(2:3,
                                               8:9,
                                               11:12,
                                               16:23,
                                               29:30,
...
entire.demo.chart["N",] = c(dim(Interpolate.Master.One.Study)[1], 
                            NA,
                            dim(Interpolate.Master.One.Control)[1],
                            NA,
                            # dim(Interpolate.Master.One)[1],
                            # NA,
                            NA)
...
for (i in cont.list[c(1:12,18:23)]){
  ...
  tmp = t.test(Interpolate.Master.One.Study[, i],
               Interpolate.Master.One.Control[, i])
  ...
  entire.demo.chart[i, 2] = round(sqrt(var(Interpolate.Master.One.Study[, i], 
                                           na.rm = TRUE)), 
                                  digits = 1)
  ...
  # place the sample standard deviation for the study group in the chart
  entire.demo.chart[i, 4] = round(sqrt(var(Interpolate.Master.One.Control[, i], 
                                           na.rm = TRUE)), 
                                  digits = 1)
  
  # # place the sample mean for the entire cohort in the chart
  # entire.demo.chart[i, 5] = round(mean(Interpolate.Master.One[, i],
  #                                      na.rm = TRUE), 
  #                                 digits = 1)
  # 
  # # place the sample standard deviation for the entire cohort in the chart
  # entire.demo.chart[i, 6] = round(sqrt(var(Interpolate.Master.One[, i], 
  #                                          na.rm = TRUE)), 
  #                                 digits = 1)
...
for (i in cont.list[c(13:17)]){
  ...
  # place the sample standard deviation for the study group in the chart
  entire.demo.chart[i, 2] = round(sqrt(var(Interpolate.Master[Interpolate.Master[, "ID"] %in% StudyGroupIDs & Interpolate.Master[,"DaysId"] == 1, i], 
                                           na.rm = TRUE)), 
                                  digits = 1)
  ...
  # place the sample standard deviation for the study group in the chart
  entire.demo.chart[i, 4] = round(sqrt(var(Interpolate.Master[Interpolate.Master[, "ID"] %in% ControlGroupIDs & Interpolate.Master[,"DaysId"] == 1, i], 
                                           na.rm = TRUE)), 
                                  digits = 1)
  
  # # place the sample mean for the entire cohort in the chart
  # entire.demo.chart[i, 5] = round(mean(Interpolate.Master[Interpolate.Master[,"DaysId"] == 1, i],
  #                                      na.rm = TRUE), 
  #                                 digits = 1)
  # 
  # # place the sample standard deviation for the entire cohort in the chart
  # entire.demo.chart[i, 6] = round(sqrt(var(Interpolate.Master[Interpolate.Master[,"DaysId"] == 1, i], 
  #                                          na.rm = TRUE)), 
  #                                 digits = 1)
...
for (i in bin.list){
  ...
  tmp.table = table(Interpolate.Master.One[, c("Group", i)])[c(2,1), c(2,1)]
  
  # construct a 1 by 2 table for the current variable
  tmp.table2 = table(Interpolate.Master.One[, i])[c(2,1)]
  
  # if (is.null(levels(Interpolate.Master.One[, i])) == 0){
  #   
  #   if( c("") %in% levels(Interpolate.Master.One[, i]) |
  #       c("9999") %in% levels(Interpolate.Master.One[, i])){ 
  #     
  #     print(i)
  #     
  #     tmp.table = table(Interpolate.Master.One[, c("Group", i)])[c(1,2), c(3,2)]
  #     
  #   }
  #   
  # }
  ...
  #   tmp.table = table(Interpolate.Master.One[, c("Group", i)])[c(2,1), c(3,1)]
  #   
  #   # construct a 1 by 2 table for the current variable
  #   tmp.table2 = table(Interpolate.Master.One[, i])[c(3,1)]
  #   
  # }
  ...
  # # place the number of patients with this binary variable in the entire cohort in the chart
  # entire.demo.chart[i, 5] = round(tmp.table2[1], 
  #                                 digits = 0)
  # 
  # # place the sample proportion for the entire cohort in the chart
  # entire.demo.chart[i, 6] = round(tmp.table2[1]/nrow(Interpolate.Master.One), 
  #                                 digits = 2)
...
tmp.table = table(Interpolate.Master.One[, c("Group", "New.Race")])[c(2,1), ]
...
for (i in colnames(tmp.table)){
  ...
  # # place the number of patients in the current category that are in the entire cohort in the chart
  # entire.demo.chart[i, 5] = table(Interpolate.Master.One[, "New.Race"])[i]
  # 
  # # place the sample proportion of patients in the current category that are in the entire cohort in the chart
  # entire.demo.chart[i, 6] = round(table(Interpolate.Master.One[, "New.Race"])[i]/entire.demo.chart["N", 5],
  #                                 digits = 2)
...
entire.demo.chart = entire.demo.chart[c(1:5,
                                        60,
                                        57:59,
                                        6:56), ]

# delete the New Race row
entire.demo.chart = entire.demo.chart[-which(rownames(entire.demo.chart) == "New.Race"), ]

# For health insurance ####

# create a table for the number of patients in each category of health insurance
tmp.table = table(Interpolate.Master.One[, c("Group", "Health.Insurance.Name")])[c(2,1), 1:3] +
  cbind(c(0,0), 
        c(0,0),
        table(Interpolate.Master.One[, c("Group", "Health.Insurance.Name")])[c(2,1), 4])

tmp.table2 = table(Interpolate.Master.One[, "Health.Insurance.Name"])[1:3] +
  c(0,0,table(Interpolate.Master.One[, "Health.Insurance.Name"])[4])
...
for (i in colnames(tmp.table)){
  ...
  # # place the number of patients in the current category that are in the entire cohort in the chart
  # entire.demo.chart[i, 5] = tmp.table2[i]
  # 
  # # place the sample proportion of patients in the current category that are in the entire cohort in the chart
  # entire.demo.chart[i, 6] = round(tmp.table2[i]/entire.demo.chart["N", 5],
  #                                 digits = 2)
...
entire.demo.chart = entire.demo.chart[c(1:10, 
                                        60:62, 
                                        11:59), ]
...
tmp.table = table(Interpolate.Master.One[, c("Group", "Type.of.Stroke")])[c(2,1), ]
...
for (i in colnames(tmp.table)){
  ...
  # # place the number of patients in the current category that are in the entire cohort in the chart
  # entire.demo.chart[i, 5] = table(Interpolate.Master.One[, "Type.of.Stroke"])[i]
  # 
  # # place the sample proportion of patients in the current category that are in the entire cohort in the chart
  # entire.demo.chart[i, 6] = round(table(Interpolate.Master.One[, "Type.of.Stroke"])[i]/entire.demo.chart["N", 5],
  #                                 digits = 2)
...
entire.demo.chart = entire.demo.chart[c(1:15, 
                                        63:65, 
                                        16:62), ]
...
entire.demo.chart = entire.demo.chart[c(1:4, 
                                        66, 
                                        5:65), ]
...
for (i in entire.demo.chart.final[, "Variables"]){
  ...
  if (i %in% cont.list){
    ...
    # # place the mean plus/minus the standard deviation for the entire cohort in the chart    
    # entire.demo.chart.final[i, "Total"] = 
    #   paste(entire.demo.chart[i, 5], 
    #         "\u00B1", 
    #         entire.demo.chart[i, 6], 
    #         sep = "")
    ...
    # # place the sample proportion and number of people with the condition for the entire cohort in the chart    
    # entire.demo.chart.final[i, "Total"] = 
    #   paste(entire.demo.chart[i, 5], 
    #         " (", 
    #         100*entire.demo.chart[i, 6], 
    #         ")", 
    #         sep = "")
...
entire.demo.chart.final[is.na(entire.demo.chart.final)] = ""
